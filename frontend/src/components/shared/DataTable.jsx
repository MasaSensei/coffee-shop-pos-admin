...
export function DataTable({
  ...
  renderRowDetails, // Tambahkan prop ini untuk custom detail
}) {
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-left">
          ...
          <tbody className="divide-y divide-stone-50">
            {data.map((row) => {
              // LOGIKA UNIVERSAL: Munculkan tombol jika ada variants, items, atau fungsi render custom
              const hasDetail =
              ...
              return (
                <>
                  ...
                  {/* LOGIKA DETAIL UNIVERSAL */}
                  {expandedRows[row.id] && (
                    <tr className="bg-stone-50/30 animate-in slide-in-from-top-2 duration-300">
                      <td colSpan={columns.length + 1}>
                        {/* 1. Jika ada renderer khusus (seperti untuk Purchasing) */}
                        {renderRowDetails
                          ? renderRowDetails(row)
                          : /* 2. Default ke Varian (untuk Products) */
                            row.variants && (
                              <div className="flex flex-col gap-2 border-l-4 border-amber-100 pl-8 py-6 mx-6">
